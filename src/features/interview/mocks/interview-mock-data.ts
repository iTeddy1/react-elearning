    ...
    beginner: [
      {
        ...
        expectedTopics: ['Variable declarations', 'Block scope', 'Hoisting', 'Temporal dead zone'],
        followUpQuestions: [
      ...
      },
      {
        ...
        difficulty: 'beginner' as const,
        expectedTopics: ['Learning process', 'Time management', 'Problem-solving', 'Adaptability'],
        followUpQuestions: [
      ...
      },
      {
        ...
        category: 'Technical',
        question: 'How would you explain what React is to someone who has never heard of it?',
        difficulty: 'beginner' as const,
        expectedTopics: ['Component-based architecture', 'Virtual DOM', 'State management', 'UI library'],
        followUpQuestions: [
      ...
      },
      {
        ...
        question: 'If a user reports that the website is loading slowly, how would you investigate and solve this issue?',
        difficulty: 'beginner' as const,
        expectedTopics: ['Performance debugging', 'Browser developer tools', 'Network analysis', 'Optimization techniques'],
        followUpQuestions: [
      ...
      },
      {
        ...
        difficulty: 'beginner' as const,
        expectedTopics: ['CSS specificity rules', 'Selector weight', 'Inheritance', 'Cascade'],
        followUpQuestions: [
    ...
    intermediate: [
      {
        ...
        expectedTopics: ['Lexical scoping', 'Function scope', 'Memory management', 'Module pattern'],
        followUpQuestions: [
      ...
      },
      {
        ...
        category: 'Technical',
        question: 'How does React\'s reconciliation algorithm work, and why is the key prop important?',
        difficulty: 'intermediate' as const,
        expectedTopics: ['Virtual DOM diffing', 'Reconciliation', 'Key prop', 'Performance optimization'],
        followUpQuestions: [
      ...
      },
      {
        ...
        difficulty: 'intermediate' as const,
        expectedTopics: ['Debugging process', 'Problem-solving', 'Tools and techniques', 'Root cause analysis'],
        followUpQuestions: [
    ...
    advanced: [
      {
        ...
        category: 'Technical',
        question: 'Design a performant infinite scroll component for a large dataset. What considerations would you make?',
        difficulty: 'advanced' as const,
        expectedTopics: ['Virtual scrolling', 'Performance optimization', 'Memory management', 'User experience'],
        followUpQuestions: [
    ...
    beginner: [
      {
        ...
        difficulty: 'beginner' as const,
        expectedTopics: ['Database types', 'ACID properties', 'Scalability', 'Data consistency'],
        followUpQuestions: [
    ...
    intermediate: [
      {
        ...
        category: 'Technical',
        question: 'How would you design a caching strategy for a high-traffic web application?',
        difficulty: 'intermediate' as const,
        expectedTopics: ['Caching layers', 'Cache invalidation', 'Redis', 'CDN', 'Database caching'],
        followUpQuestions: [
    ...
    advanced: [
      {
        ...
        category: 'Technical',
        question: 'Design a distributed system that can handle millions of users. What are your key architectural considerations?',
        difficulty: 'advanced' as const,
        expectedTopics: ['Scalability', 'Load balancing', 'Database sharding', 'Microservices', 'CAP theorem'],
        followUpQuestions: [
    ...
    beginner: [
      {
        ...
        difficulty: 'beginner' as const,
        expectedTopics: ['Client-server communication', 'HTTP requests', 'APIs', 'JSON'],
        followUpQuestions: [
    ...
    intermediate: [
      {
        ...
        category: 'Technical',
        question: 'Describe how you would implement user authentication in a full-stack application.',
        difficulty: 'intermediate' as const,
        expectedTopics: ['JWT tokens', 'Session management', 'Password hashing', 'Security best practices'],
        followUpQuestions: [
    ...
    advanced: [
      {
        ...
        category: 'Technical',
        question: 'Design the architecture for a real-time collaborative application like Google Docs.',
        difficulty: 'advanced' as const,
        expectedTopics: ['Real-time communication', 'WebSockets', 'Conflict resolution', 'Operational transformation'],
        followUpQuestions: [
...
export const mockAnswerAnalysis: Record<string, AnswerAnalysis> = {
  'fe-bg-001': {
    transcription: "Let, const, and var are all ways to declare variables in JavaScript. Var is the old way and has function scope, while let and const have block scope. Const is for constants that cannot be reassigned, and let is for variables that can be changed.",
    score: 78,
    analysis: {
      relevance: 85,
    ...
    feedback: "Good basic understanding of variable declarations. The explanation covers the main differences but could be enhanced with examples and deeper concepts like hoisting."
  }
};
...
export const getMockQuestionsByRole = (
  ...
  const normalizedRole = role.toLowerCase().includes('frontend') ? 'Frontend Developer' :
                         role.toLowerCase().includes('backend') ? 'Backend Developer' :
                         role.toLowerCase().includes('full') ? 'Full Stack Developer' :
  ...
  const questions = mockInterviewQuestions[normalizedRole]?.[mockDifficulty] || mockInterviewQuestions['Frontend Developer'][mockDifficulty];
...
export const getMockAnswerAnalysis = (questionId: string): AnswerAnalysis => {
  return mockAnswerAnalysis[questionId] || {
    transcription: "This is a sample transcription of the candidate's response to the interview question. The candidate provided a thoughtful answer that demonstrates understanding of the topic.",
    ...
    feedback: "Good overall response demonstrating solid understanding. With more practice and specific examples, the candidate could significantly improve their interview performance."
  };
};
...
export const getMockInterviewReview = (
  ...
  const basicFeedback = mockInterviewReviews[key] || mockInterviewReviews['frontend-beginner-good'];
  
  // Convert to AI service's InterviewFeedback format
  const overallScore = basicFeedback.overallScore;
  const decision = overallScore >= 85 ? 'RECOMMEND' : 
                   overallScore >= 70 ? 'MAYBE' : 'NOT_RECOMMEND';
  
  return {
    overallScore: overallScore,
    scores: {
      technicalKnowledge: Math.max(60, overallScore - 10 + Math.floor(Math.random() * 15)),
      communicationSkills: Math.max(60, overallScore - 5 + Math.floor(Math.random() * 10)),
      problemSolving: Math.max(60, overallScore - 8 + Math.floor(Math.random() * 12)),
      professionalism: Math.max(60, overallScore + 5 + Math.floor(Math.random() * 8)),
      overallFit: Math.max(60, overallScore - 3 + Math.floor(Math.random() * 10))
    },
    questionAnalysis: [
      {
        questionIndex: 0,
