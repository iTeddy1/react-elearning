      ...
      (set, get) => ({
        ...
        updateUserProgress: () => {
          ...
          const correctAnswers = attempts.reduce((sum, attempt) => {
            return (
              sum +
              Math.round((attempt.percentage / 100) * attempt.totalQuestions)
            );
          }, 0);
        ...
        updateTopicProgress: (topicId: number, topicName: string) => {
          const state = get();
          const topicAttempts = state.quizAttempts.filter(
            (attempt) => attempt.topicId === topicId
          );
          ...
          const bestScore = Math.max(
            ...topicAttempts.map((attempt) => attempt.percentage)
          );
          const totalTimeSpent = topicAttempts.reduce(
            (sum, attempt) => sum + attempt.timeSpent,
            0
          );
          ...
          const existingProgressIndex = state.topicProgress.findIndex(
        ...
        calculateStreak: () => {
          const state = get();
          const attempts = state.quizAttempts.sort(
            (a, b) =>
              new Date(b.completedAt).getTime() -
              new Date(a.completedAt).getTime()
          );
        ...
        getProgressByDifficulty: (difficulty: string) => {
          ...
          const difficultyAttempts = state.quizAttempts.filter(
            (attempt) =>
              attempt.difficulty?.toLowerCase() === difficulty.toLowerCase()
          );
...
export const useProgressSelectors = () => {
  ...
  return {
    ...
    accuracy:
      store.userProgress.totalQuestionsAnswered > 0
        ? Math.round(
            (store.userProgress.correctAnswers /
    ...
    averageTimePerQuiz:
      store.userProgress.totalQuizzesCompleted > 0
        ? Math.round(
            store.userProgress.totalTimeSpent /
              store.userProgress.totalQuizzesCompleted
          )
        : 0,
    ...
    difficultyBreakdown: {
      beginner: store.quizAttempts.filter(
        (attempt) => attempt.difficulty?.toLowerCase() === 'beginner'
      ).length,
      intermediate: store.quizAttempts.filter(
        (attempt) => attempt.difficulty?.toLowerCase() === 'intermediate'
      ).length,
      advanced: store.quizAttempts.filter(
        (attempt) => attempt.difficulty?.toLowerCase() === 'advanced'
      ).length,
    },
  };
};
