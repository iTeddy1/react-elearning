...
export const InterviewSetupPage: React.FC = () => {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      if (data.resumeFile) {
        try {
          const result = await extractResumeText(data.resumeFile);
          resumeText = formatResumeForAI(result.text);
          console.log('📄 Resume extracted successfully');
          console.log('📊 Resume metadata:', result.metadata);
          toast.success('Resume processed successfully', {
            description: `${result.metadata.wordCount} words extracted`,
            duration: 2000,
          });
        } catch (error) {
          console.error('Failed to extract resume text:', error);
          toast.warning('Failed to read resume file', {
            description: 'Continuing without resume context',
            duration: 3000,
          });
        }
      }
