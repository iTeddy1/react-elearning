...
export const useGenerateInterviewQuestionsMutation = (callbacks?: {
  ...
  return useMutation({
    ...
    onError: (error: Error) => {
      console.error('❌ InterviewReview generation failed:', error.message);

      // Determine user-friendly error message
      let errorTitle = 'Failed to generate interview review';
      let errorDescription = 'Please try again in a few moments';

      if (error.message.includes('parse') || error.message.includes('JSON')) {
        errorTitle = 'AI Response Format Error';
        errorDescription =
          "The AI returned an invalid format. We've fallen back to a standard review.";
      } else if (
        ...
        errorTitle = 'No Response from AI';
        errorDescription =
          "The AI service didn't respond. Using fallback review system.";
      } else if (error.message.includes('timeout')) {
        errorTitle = 'Request Timeout';
        errorDescription =
          'The review is taking too long. Using fallback system.';
      } else if (error.message.includes('Missing required field')) {
        errorTitle = 'Incomplete AI Response';
        errorDescription =
          'The AI response was incomplete. Using fallback with available data.';
      }

      // Show error toast with recovery information
      toast.error(errorTitle, {
        description: errorDescription + ' Your interview data is safe.',
        duration: 7000,
      });
